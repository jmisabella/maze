...
class WilsonsSpec extends AnyFlatSpec with GivenWhenThen {
  ...
  it should "honor start and goal coordinates specified in MazeRequest when generating a 5x10 Wilson's orthogonal maze grid" in {
  ...
  it should "honor start and goal coordinates specified in MazeRequest when generating a 5x10 Wilson's delta maze grid" in {
    Given("5x10 Wilsons request")
    val request = MazeRequest(Delta, 5, 10, Algorithm.Wilsons, Coordinates(0, 9), Coordinates(4, 0))
    When("generating the grid")
    var grid = Generator.generate(request)
    Then("grid's start should be southwest")
    grid.startCoords should equal (Coordinates(0, request.height - 1))
  ...
  it should "honor start and goal coordinates specified in MazeRequest when generating a 5x10 Wilson's sigma maze grid" in {
    Given("5x10 Wilsons request")
    val request = MazeRequest(Sigma, 5, 10, Algorithm.Wilsons, Coordinates(0, 9), Coordinates(4, 0))
    When("generating the grid")
    var grid = Generator.generate(request)
    Then("grid's start should be southwest")
    grid.startCoords should equal (Coordinates(0, request.height - 1))
